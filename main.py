...
training_status_db = {} # { email: status } – e.g., "not started", "in progress", "completed"
...
async def ingest_data(data: DataIngestion):
    if data.source not in data_store:
        data_store[data.source] = []
    data_store[data.source].append(data.data)
...
@app.post("/iot/ingest")
async def iot_ingest(data: DataIngestion):
    if data.source not in data_store:
        data_store[data.source] = []
    data_store[data.source].append(data.data)
    return {"message": "IoT data ingested successfully"}
